# Privacy Policy

Last updated: 2026-07-27

This Privacy Policy describes how DShare ("we", "us", "our") collects, uses, and shares information when you use the DShare app and website (the "Service").

If you do not agree with this policy, do not use the Service.

## Summary

- We collect the data you provide (like email) to create and manage your account.
- We store the files and text you upload so you can access them from another device.
- We use cookies/session storage for login and security.
- We do not sell your personal information.

## Information We Collect

### Information you provide

- Account email address.
- Files and text you upload to share.
- Optional security data like passkeys (WebAuthn credentials).

### Information collected automatically

- Basic device and browser information (for example, IP address, user agent, and timestamps) in server logs.
- Cookies and similar technologies for session and CSRF protection.
- If enabled by the operator, a best-effort action/audit log (which action was taken — e.g. upload, download, clear, register, login — plus IP, user agent, timestamp, and, for logged-in actions, your account email) sent to a private, operator-only alerting channel. This never includes the content of uploaded files or shared text, and never includes passwords, PINs, or security tokens. We do not perform IP-to-location lookups or share this data with any third party.

### Third‑party services

We may use hosting and storage providers to operate the Service (for example, cloud hosting, databases, and object storage). These providers may process data on our behalf under their own privacy and security terms.

## How We Use Information

We use information to:

- Provide the Service (store and deliver your uploaded files/text).
- Authenticate users and secure accounts.
- Prevent abuse and maintain reliability.
- Comply with legal obligations.

## How We Share Information

We do not sell your personal information.

We may share information:

- With service providers who help us run the Service.
- If required by law, legal process, or to protect safety and security.

## Data Retention

Uploaded content is retained according to the app’s retention settings and may be deleted automatically after a period of time. Account data is retained as long as your account remains active or as needed to provide the Service.

## Security

We use reasonable safeguards to protect information. No method of transmission or storage is 100% secure.

## Children’s Privacy

The Service is not directed to children under 13. We do not knowingly collect personal information from children.

## Your Choices

You can request deletion of your account and data by contacting us at the email below.

## Changes to This Policy

We may update this policy from time to time. We will update the "Last updated" date when changes are made.

## Contact

If you have questions about this policy or want to request deletion, contact:

Email: divyesh@divyeshvishwakarma.com
